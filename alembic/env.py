...
Loads the SQLAlchemy URL from ``core.config.settings`` so the connection
string stays in ``.env`` rather than being duplicated in ``alembic.ini``.
"""
...
# Override the sqlalchemy.url with the real async URL from our settings.

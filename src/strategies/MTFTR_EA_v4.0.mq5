...
//| Version: 4.0  (Growth Optimized + Monthly Withdrawal)            |
//+------------------------------------------------------------------+
//| v4.0 CHANGES FROM v3.0:                                           |
//|                                                                   |
//| PROFIT MULTIPLIERS:                                               |
//|  1. Risk: 2% per trade (was 1%) — doubles every P&L number      |
//|  2. 3-tier TP: 25% at 1.5R, 35% at 3R, trail 40% to 6R+        |
//|     - TP1 at 1.5R hits faster than old 2R (improves win rate)    |
//|     - TP3 target at 6R for runners (new tier)                    |
//|  3. SL min reduced: 1.0 ATR (was 1.5) — tighter stops from      |
//|     proximity filter, bigger lots, closer TPs in points          |
//|  4. ADX(14) H1 trending filter — skip ranging markets that       |
//|     produce time exits (was 36.5% of all trades)                 |
//|  5. After TP1: SL to +1.0R (was +0.5R) — lock more profit       |
//|                                                                   |
//| CAPACITY INCREASE:                                                |
//|  - MaxOpenTrades: 3 (was 2)                                      |
//|  - MaxDailyTrades: 5 (was 3)                                     |
//|  - Session: 7-16 continuous (closed 12-13 gap)                   |
//|  - MaxConsecLosses: 4 (was 3) — less pausing                    |
//|                                                                   |
//| MONTHLY WITHDRAWAL SIMULATION:                                    |
//|  - Tracks monthly starting balance                               |
//|  - At month rollover: profit = balance - monthStartBalance       |
//|  - Withdraws 70% of profit (simulated by reducing balance base)  |
//|  - 30% compounds into account                                    |
//|  - CSV logs withdrawal events                                    |
//|                                                                   |
...
//+------------------------------------------------------------------+
#property copyright "MTFTR Strategy v4.0"
...
input double   InpRiskPercent      = 2.0;       // Risk per trade (%) [v4: was 1.0]
input int      InpMaxOpenTrades    = 3;         // Max simultaneous trades [v4: was 2]
input int      InpMaxDailyTrades   = 5;         // Max trades per day [v4: was 3]
input int      InpMaxConsecLosses  = 4;         // Consecutive losses -> pause [v4: was 3]
input double   InpMaxDailyDD       = 4.0;       // Max daily drawdown (%) [v4: was 3]
input double   InpMaxWeeklyDD      = 8.0;       // Max weekly drawdown (%) [v4: was 6]
input double   InpMaxSpreadATR     = 0.15;      // Max spread as fraction of 15M ATR

input group "====== [v4] Monthly Withdrawal ======"
...
input group "====== [v4] ADX Trend Strength Filter ======"
input bool     InpADXEnabled       = true;       // Enable ADX filter (kills ranging time exits)
input int      InpADX_Period       = 14;         // ADX period
input double   InpADX_Min          = 20.0;       // Min ADX for entry (20 = trending)
input ENUM_TIMEFRAMES InpADX_TF    = PERIOD_H1;  // ADX timeframe

input group "====== 15M Entry Settings ======"
input int      InpM15_EMA_Period   = 21;
input int      InpM15_RSI_Period   = 14;
input int      InpM15_ATR_Period   = 14;
input double   InpRSI_Long_Min     = 40.0;
input double   InpRSI_Long_Max     = 55.0;
input double   InpRSI_Short_Min    = 45.0;
input double   InpRSI_Short_Max    = 65.0;
input int      InpSwingLookback    = 3;

input group "====== Entry Precision ======"
input double   InpProximityATR     = 0.5;       // Max distance from EMA21 in M15 ATR multiples
input int      InpSwingLookbackSL  = 5;         // [v4] Swing lookback for SL (wider = more structure)

input group "====== Fibonacci Entry (Method C) ======"
input bool     InpFibEnabled       = true;
...
input double   InpFibRSI_Long_Max  = 60.0;
input double   InpFibRSI_Short_Min = 40.0;
input double   InpFibRSI_Short_Max = 65.0;

input group "====== Stop Loss ======"
input double   InpSL_ATR_Buffer    = 0.3;       // ATR buffer [v4: was 0.5]
input double   InpSL_Max_ATR       = 3.0;       // Max SL width [v4: was 4.0]
input double   InpSL_Min_ATR       = 1.0;       // Min SL width [v4: was 1.5]

input group "====== [v4] 3-Tier Take Profit ======"
input double   InpTP1_RR           = 1.5;       // TP1 R:R [v4: was 2.0]
input double   InpTP2_RR           = 3.0;       // TP2 R:R [v4: was 4.0]
input double   InpTP3_RR           = 6.0;       // [v4] TP3 R:R target (trail runner)
input double   InpTP1_ClosePct     = 25.0;      // % at TP1 [v4: was 30]
input double   InpTP2_ClosePct     = 35.0;      // % at TP2 [v4: was 40]
input double   InpTP1_SL_MoveToRR  = 1.0;       // After TP1: SL to +1.0R [v4: was 0.5]
input double   InpTP2_SL_MoveToRR  = 2.0;       // [v4] After TP2: SL to +2.0R
input int      InpMaxTradeHours    = 16;        // Time exit hours [v4: was 24]
input bool     InpSmartTimeExit    = true;

input group "====== Session Filter ======"
input int      InpGMTOffset        = 0;
input int      InpSessionStart     = 7;         // [v4] Single session window start
input int      InpSessionEnd       = 16;        // [v4] Single session window end (closed gap)
input bool     InpFridayFilter     = true;

input group "====== General ======"
input int      InpMagicNumber      = 20260404;  // [v4] New magic

input group "====== Logging ======"
input bool     InpEnableCSV        = true;
input string   InpCSVFileName      = "MTFTR_v4_TradeLog.csv";
...
// [v4] Monthly withdrawal tracking
int    currentMonth = 0;
...
int g_posCount = 0;
string g_panelName = "MTFTR4_Panel";
...
int OnInit()
{
   ...
   // Auto-detect a broker-supported filling mode.
   ...
   { Print("MTFTR ERROR: Indicator handles failed"); return INIT_FAILED; }
   ...
   g_safetyPrefix = "MTFTR4_S_" + _Symbol + "_";
   
   ArrayResize(g_positions,10);
   LoadExistingPositions();
   LoadSafetyState();
   
   Print("MTFTR v4.0 | Risk=",InpRiskPercent,"% | TP=",InpTP1_RR,"/",InpTP2_RR,"/",InpTP3_RR,
         "R | ADX>",InpADX_Min," | Withdraw=",InpWithdrawPct,"%");
...
//+------------------------------------------------------------------+
//| [v4] Monthly Withdrawal Simulation                               |
//+------------------------------------------------------------------+
void CheckMonthlyWithdrawal()
{
   ...
   if(thisMonth != currentMonth)
   {
      ...
      if(monthProfit > 0)
      {
         ...
         // In backtesting we can't actually withdraw, so we track it
         // The 30% compounds naturally since we don't reduce balance
         // But we LOG the withdrawal amount for tracking
         Print("MTFTR WITHDRAWAL: Month ", currentMonth,
               " | Profit: $", DoubleToString(monthProfit, 2),
               " | Withdraw 70%: $", DoubleToString(withdrawal, 2),
      ...
      {
         Print("MTFTR: Month ", currentMonth, " closed at loss: $",
...
//+------------------------------------------------------------------+
//| Bias Functions (same as v3.0 — bar[1] confirmed)                 |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| 15M Entry + Fibonacci (same as v3.0)                             |
//+------------------------------------------------------------------+
...
void ExecuteTrade(ENUM_TREND_BIAS bias, ENUM_ENTRY_METHOD method)
{
   ...
   bool ok=(bias==BIAS_LONG)?trade.Buy(lots,_Symbol,entry,sl,0,"MTFTR4_"+mc):trade.Sell(lots,_Symbol,entry,sl,0,"MTFTR4_"+mc);
   ...
   if(ok&&trade.ResultRetcode()==TRADE_RETCODE_DONE)
   {
      ...
      Print("MTFTR OPEN: ",(bias==BIAS_LONG?"LONG":"SHORT")," | ",mf,
   ...
   else Print("MTFTR ERROR: ",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());
}

//+------------------------------------------------------------------+
//| Position Management — 3-tier exit                                |
//+------------------------------------------------------------------+
void ManageOpenPositions()
{
   for(int i=0;i<g_posCount;i++)
   {
      ...
      // --- Time exit with 10018 protection
      if(st==STATE_INITIAL)
      {
         ...
         if(elapsed>InpMaxTradeHours*3600)
         {
            ...
            { g_positions[i].entryTime+=InpMaxTradeHours*3600; g_positions[i].timeExtended=true;
              Print("MTFTR: #",tk," in profit (",DoubleToString(unPnL,_Digits),"). Extending."); continue; }
            if(!IsMarketOpen())
      ...
      if(st==STATE_INITIAL)
      {
         ...
         if(hit)
         {
            ...
            if(cv>=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN)&&cv<pv)
            {
               ...
               {
                  double newSL=g_positions[i].entryPrice+dir*slD*InpTP1_SL_MoveToRR;
                  ...
                  Print("MTFTR TP1(",InpTP1_RR,"R): -",cv," lots. SL→+",InpTP1_SL_MoveToRR,"R (",DoubleToString(newSL,_Digits),") #",tk);
         ...
         if(hit)
         {
            ...
            if(tv>=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN)&&cv>0)
            {
               if(trade.PositionClosePartial(tk,cv))
               {
                  ...
                  Print("MTFTR TP2(",InpTP2_RR,"R): -",cv," lots. SL→+",InpTP2_SL_MoveToRR,"R. Trail ",tv," #",tk);
      ...
      // --- TP3 target + H4 Hull trail
      else if(st==STATE_TP2_HIT)
      {
         // Check TP3 target hit — close everything
         ...
         if(tp3Hit)
         {
            ...
            g_positions[i].active=false;
            Print("MTFTR TP3(",InpTP3_RR,"R): FULL CLOSE at ",DoubleToString(px,_Digits)," #",tk);
            continue;
         }
         ...
         if(h4t!=g_positions[i].lastH4Check)
         {
            ...
            if(flip)
            {
               ...
               g_positions[i].active=false;
               Print("MTFTR H4 Trail exit #",tk," at ",DoubleToString(px,_Digits)); continue;
            }
              ...
              double vol,int dir,string method,double slD)
{
   ...
   string p="MTFTR_"+IntegerToString(tk);
...
void LoadExistingPositions()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      ulong tk=posInfo.Ticket(); string p="MTFTR_"+IntegerToString(tk);
...
void CleanupClosedPositions()
{
   for(int i=0;i<g_posCount;i++)
   {
      ...
      { consecLosses++; LogEvent("SL_HIT",g_positions[i],g_positions[i].lastKnownSL,"");
        if(consecLosses>=InpMaxConsecLosses){dailyPauseActive=true;Print("MTFTR: Paused after ",consecLosses," losses");}}
      else
      { consecLosses=0;
        if(g_positions[i].state==STATE_TP1_HIT) LogEvent("BE_EXIT",g_positions[i],g_positions[i].lastKnownSL,"+1R SL triggered");
...
{if(dayStartBalance<=0)return true;double dd=(dayStartBalance-AccountInfoDouble(ACCOUNT_BALANCE))/dayStartBalance*100;
 if(dd>=InpMaxDailyDD){static datetime lp=0;if(TimeCurrent()-lp>300){Print("MTFTR: Daily DD ",DoubleToString(dd,1),"%");lp=TimeCurrent();}return false;}return true;}
...
{if(weekStartBalance<=0)return true;double dd=(weekStartBalance-AccountInfoDouble(ACCOUNT_BALANCE))/weekStartBalance*100;
 if(dd>=InpMaxWeeklyDD){static datetime lp=0;if(TimeCurrent()-lp>300){Print("MTFTR: Weekly DD ",DoubleToString(dd,1),"%");lp=TimeCurrent();}return false;}return true;}
...
void UpdateInfoPanel()
{
   ...
   {ObjectCreate(0,bg,OBJ_RECTANGLE_LABEL,0,0,0);
    ...
    ObjectSetInteger(0,bg,OBJPROP_XSIZE,180);ObjectSetInteger(0,bg,OBJPROP_YSIZE,260);
   ...
   PL(g_panelName+"_T","MTFTR v4.0 [2% 1.5/3/6R]",165,y,clrWhite,9,true); y+=s+4;
   ...
   PL(g_panelName+"_D1","D1:"+ds+" | 4H:"+bs,165,y,
      (b4==BIAS_LONG)?InpBullColor:(b4==BIAS_SHORT)?InpBearColor:InpNeutralColor,9,true); y+=s;
   ...
   PL(g_panelName+"_TL","Trailing: "+IntegerToString(tr)+" | Open: "+IntegerToString(CountOpenPositions()),165,y,clrWhite,9,false);
}
